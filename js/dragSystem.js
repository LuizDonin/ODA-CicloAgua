...
export default class dragSystem extends Phaser.Scene {
    ...
    create() {
      ...
      // feedback helpers
      this.clearFeedback = () => {
        if (this.feedbackTimer) { this.feedbackTimer.remove(); this.feedbackTimer = null; }
        if (this.feedbackIcon) { this.feedbackIcon.destroy(); this.feedbackIcon = null; }
      };
      ...
      this.showFeedback = (key) => {
        ...
        this.feedbackIcon = this.add.image(cx, cy, key)
          .setOrigin(0.5)
          .setDepth(2000)
          .setScale(0.9)
          .setAlpha(0);
  
        this.tweens.add({ targets: this.feedbackIcon, alpha: 1, duration: 120, ease: 'Quad.easeOut' });
        this.feedbackTimer = this.time.delayedCall(1500, () => this.clearFeedback());
      };
      ...
      this.input.on('drop', (pointer, obj, zone) => {
        ...
        if (hitId && hitId === mustId) {
          ...
          this.tweens.add({
            ...
            ease: 'Quad.easeOut',
            onComplete: () => {
              this.showFeedback(fbKeys.correct);
            }
          });
